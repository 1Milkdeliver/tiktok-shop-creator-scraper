  ...
  async start(config) {
    ...
    try {
      ...
      if (config.libraryUpdate) {
        this.log(`✅ 达人库更新完成：${rows.length} 位达人`);
        ...
        this.log(`✅ 抓取完成：${rows.length} 位达人人已存入达人库（未自动导出文件）`);
          ...
          res = await scrapeByInput(s.page, inputs, {
            ...
            onNewCreators: async (newOnes) => {
              ...
              for (const c of newOnes) {
                ...
                // hard cap so a wedged page can never stall the whole loop
                try { await Promise.race([fetchOneDetail(c), this.sleep(90000).then(() => { this.log(`  会话#${s.index + 1} 单条详情超时，跳过`); }) ]); } catch (e) { }
