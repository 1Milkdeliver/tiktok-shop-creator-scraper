...
// After this many details, a session closes its browser and reopens with the
// same cookie to release accumulated Chrome memory on very long runs (~1-2h).
const MEMORY_RESTART_DETAILS = 600;
...
const FIELD_LABELS = {
  ...
  '垂直类目': { zh: '垂直类目', en: 'Vertical Category' },
...
class MultiRunner {
  constructor() {
    ...
    this._rateState = { lastTotal: 0, lastAt: 0, ratePerMin: 0 }; // speed monitor
    this.logs = [];
    this.result = null;
    this.sessions = [];
  ...
  // resume + refresh every session page first (leaves any captcha/error page and
  // gives fetches a clean context). Called by the UI "continue" and by the
  // auto-detector once the slider is solved.
  async resumeWithRefresh() {
    ...
    for (const s of this.sessions || []) {
      ...
      try {
        await Promise.race([
          s.page.reload({ waitUntil: 'domcontentloaded', timeout: 60000 }),
          new Promise(r => setTimeout(r, 65000)),
        ]).catch(() => { });
  ...
  // sit and watch. While paused we ALSO poll the pages every ~12s: if the user
  // manually solves the slider, we detect it and auto-continue immediately
  // (no need to wait for the full cool-down or click anything).
  scheduleAutoResume(minutes) {
    this.autoResumeAt = Date.now() + (minutes || 30) * 60000;
    this.log(`⏰ 风控冷却 ${minutes || 30} 分钟后自动继续（${new Date(this.autoResumeAt).toLocaleTimeString()}）——期间可手动完成滑块验证，检测到后将立即继续`);
    if (!this._autoResumeTimer) {
      this._autoResumeTimer = setInterval(async () => {
        ...
        if (this.paused && this.autoResumeAt && Date.now() >= this.autoResumeAt) {
          ...
          this.resumeWithRefresh().catch(() => { });
          return;
        }
        // slider detection: while cooling down, check if any session page has
        // left the captcha state (URL no longer captcha/verify). If so, the
        // user solved it → refresh + resume right away.
        if (this.paused) {
          const solved = await this.captchaSolved();
          if (solved) {
            this.clearAutoResume();
            this.log('✅ 检测到滑块验证已完成，立即自动继续');
            this.resumeWithRefresh().catch(() => { });
          }
        }
      }, 5000);
  ...
  // Returns true when every open session page has exited the captcha / verify
  // URL state (i.e. the user has solved the slider or navigated away).
  async captchaSolved() {
    const sessions = this.sessions || [];
    let checked = 0;
    let solved = 0;
    for (const s of sessions) {
      if (!s.page || s.page.isClosed()) { solved++; continue; }
      try {
        const url = await Promise.race([
          s.page.evaluate(() => location.href),
          new Promise(r => setTimeout(() => r(''), 6000)),
        ]);
        if (url === null || url === undefined) continue;
        checked++;
        if (typeof url === 'string' && url && !/(captcha|verify|checkpoint|login)/i.test(url)) solved++;
      } catch (e) { solved++; } // unreadable page counts as "no captcha blocking"
    }
    // only conclude "solved" if we could inspect at least one page and none had captcha
    return checked > 0 && solved === checked;
  }
  clearAutoResume() {
  ...
  async start(config) {
    ...
    this.logs = [];
    this.result = null;
    this.currentInfo = { keyword:'', page:0, total:Array.isArray(config.creatorInput) ? config.creatorInput.length : 0, completed:0, libraryUpdate:!!config.libraryUpdate };
    this._rateState = { lastTotal: 0, lastAt: 0, ratePerMin: 0 };
    ...
    try {
      ...
      const startSession = async (i) => {
        const s = sessions[i];
        try {
          const opened = await this.openSession(mode, s.cookieFile, i, isolated);
          s.browser = opened.browser;
          s.profileDir = opened.profileDir;
          // stop may have landed while the browser was launching — bail now
          if (this.stopped) {
            try { s.browser.close().catch(() => { }); } catch (e) { }
            s.done = true;
            return;
          }
          const lp = await openLandingPage(s.browser, s.cookieFile, () => this.stopped);
          s.page = lp.page;
          ...
          const st = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('login-check-timeout')), 15000)),
          ]);
      ...
      const startupPromises = sessions.map((_, i) => (async () => {
        // interruptible stagger: chunked sleep so a stop click lands promptly
        const target = i * stagger;
        await this.interruptibleSleep(target);
        if (this.stopped) { sessions[i].done = true; return; }
      ...
      let lastDbSyncAt = Date.now();
      while (true) {
        const active = sessions.filter(s => !s.done);
        if (active.length === 0) break;
        if (this.stopped) {
          if (!stopWaitStart) stopWaitStart = Date.now();
          if (Date.now() - stopWaitStart > 30000) {
            this.log('⚠️ 等待会话结束超时，直接导出已抓取数据');
            break;
          }
        }
        // ── periodic library sync: every ~30s, persist whatever all sessions
        //    have collected so far, so a long run never loses data to a crash
        //    and the Creator Library stays fresh while scraping is running.
        if (!testMode && !this.stopped && Date.now() - lastDbSyncAt >= 30000 && this.onDataReady) {
          ...
          try {
            ...
            for (const s of sessions) {
              for (const c of s.creators || []) {
                const key = c && c.creator_oecuid == null ? '' : String(c && c.creator_oecuid);
                if (key && !syncSeen.has(key)) { syncSeen.add(key); syncCreators.push(c); }
              }
            }
            const syncDetails = [];
            for (const s of sessions) syncDetails.push(...(s.details || []));
            const syncRows = mergeRows(syncCreators, syncDetails).map(applyActivity);
            if (syncRows.length) {
              const dbRes = await this.onDataReady(syncRows, config) || {};
              this.log(`💾 自动同步达人库（进行中）：${syncRows.length} 条（新增 ${dbRes.inserted || 0} · 更新 ${dbRes.updated || 0}）`);
            }
          } catch (e) {
            this.log(`⚠️ 定时同步达人库失败: ${e.message}`);
          }
        }
        // poll faster after a stop request so the app exits promptly
        await this.sleep(this.stopped ? 1000 : 5000);
      }
      ...
      if (config.libraryUpdate) {
        const upd = Number(database.inserted) || 0;
        const ref = Number(database.updated) || 0;
        this.log(`✅ 达人库更新完成：${rows.length} 位达人（🆕 新增 ${upd.toLocaleString()} · 🔄 更新 ${ref.toLocaleString()}）`);
        this.result = { ok:true, rows:rows.length, outPath:'', creators:allCreators.length, details:allDetails.length, sessions:sessions.length, database, libraryUpdate:true };
        return;
      } else if (testMode) {
      ...
      } else if (config.autoExport === false) {
        // Scrape page only fills the library — no CSV/Excel is written here.
        // The user exports on demand from the Creator Library / Export Center.
        const inserted = Number(database.inserted) || 0;
        const updated = Number(database.updated) || 0;
        const dbNote = inserted || updated ? `（🆕 新增 ${inserted.toLocaleString()} 位 · 🔄 更新 ${updated.toLocaleString()} 位）` : '';
        this.log(`✅ 抓取完成：${rows.length} 位达人已存入达人库${dbNote}（未自动导出文件）`);
        this.result = { ok: true, rows: rows.length, outPath: '', creators: allCreators.length, details: allDetails.length, sessions: sessions.length, database, libraryOnly: true };
      } else {
      ...
      // Report which cookies were CONFIRMED invalid (landed on login/blank page)
      // so the UI can auto-remove them. Cookies merely "expired by date" but
      // still working are NOT listed here — they are kept.
      if (this.result && this.result.ok) {
        this.result.doneAt = Date.now();
        this.result.invalidCookieIndexes = sessions.filter(s => s.cookieInvalid).map(s => s.index);
      }
      ...
      // ── cleanup caches from this run ──
      // 1) per-session resume files (list/detail progress) — data is now in the
      //    library, so stale resumes are deleted instead of accumulating.
      // 2) Chrome temp profile dirs created for this run (tiktok-pack-* / headless).
      const outDir = /\.(csv|xlsx)$/i.test(outPath) ? path.dirname(outPath) : outPath;
      const kwHash = keywords.slice().sort().join('|');
      for (const s of sessions) {
        try {
          if (fs.existsSync(path.join(outDir, `.resume-list-${s.index}-${hashStr(kwHash)}.json`))) fs.unlinkSync(path.join(outDir, `.resume-list-${s.index}-${hashStr(kwHash)}.json`));
        } catch (e) { }
        try {
          if (fs.existsSync(path.join(outDir, `.resume-detail-${s.index}-${hashStr(kwHash)}.json`))) fs.unlinkSync(path.join(outDir, `.resume-detail-${s.index}-${hashStr(kwHash)}.json`));
        } catch (e) { }
        // Chrome temp profiles (fresh per launch); remove after close
        if (s.profileDir && fs.existsSync(s.profileDir)) {
          try { fs.rmSync(s.profileDir, { recursive: true, force: true }); } catch (e) { }
        }
      }
      this.log('已完成，浏览器窗口已关闭。运行缓存已清理，达人库已保存最新数据。');
  ...
  async runSession(s, keywords, detail, outPath, testMode, dedupe, creatorInput, seenFile, scrapedIds, sharedSeen, sharedDetailSeen, resumeEnabled, mode, sessionsIsolated) {
    // speed-monitor phase hint: detail mode is inherently slower per creator
    ...
    try {
      ...
      const fetchOneDetail = async (c) => {
        ...
        // Hard cap: a wedged profile call must never stall the loop forever.
        // The timer is CANCELED once the call completes, so no late "timeout"
        // log fires for a detail that actually succeeded.
        let timer = null;
        let timedOut = false;
        const timeout = new Promise((_, reject) => {
          timer = setTimeout(() => {
            timedOut = true;
            this.log(`  会话#${s.index + 1} 单条详情超时，跳过（${cid}）`);
            reject(new Error('detail-timeout'));
          }, 90000);
        });
        try {
          await Promise.race([runDetail(), timeout]);
          return true;
        } catch (e) {
          return false; // timeout or fetch failure → skip this creator
        } finally {
          timedOut = true;
        ...
        async function runDetail() {
          ...
          if (j.err || j.code !== 0) {
            ...
            if (code === 16901007) {                    // rate limited → slow down
              await this.interruptibleSleep(15000);
              continue;
            }
            if (code === 100000) {                      // intermittent throttle → retry
              await this.interruptibleSleep(6000 + Math.random() * 6000);
              continue;
            }
        ...
        detailCount++;
        this.currentInfo.completed = (this.currentInfo.completed || 0) + 1;
      ...
      // ── collect phase with session rotation ──
      // Two reasons to rotate a session:
      //   1. quota exhausted / risk control (code=10000) → new session, possibly
      //      a backup cookie
      //   2. MEMORY: a long run accumulates Chrome memory (each page holds the
      //      whole SPA). After ~600 details we close the browser and open a fresh
      //      one with the SAME cookie — resume state guarantees nothing is lost
      //      and no creator is fetched twice.
      let res = null;
      let rotatedCookie = false;
      for (let rot = 0; rot < 4 && !this.stopped; rot++) {
        if (rot > 0) {
          // persist whatever this session collected so far into the library
          // BEFORE rotating — a long run must not lose data to a memory restart
          // or crash mid-way (resume files still exist, but the DB is canonical)
          try {
            const sessionRows = mergeRows(s.creators || [], s.details || []).map(applyActivity);
            if (!testMode && this.onDataReady && sessionRows.length) {
              const dbRes = await this.onDataReady(sessionRows, config);
              this.log(`💾 内存刷新前已同步 ${dbRes?.saved || 0} 条到达人库`);
            }
          } catch (e) { this.log(`⚠️ 轮换前同步达人库失败: ${e.message}`); }
          ...
          const memoryRestart = !res || !res.rateLimited;
          this.log(`⚠️ 会话 #${s.index + 1} ${memoryRestart ? `达到内存刷新阈值（已抓 ${detailCount} 条详情），重启浏览器释放内存` : '配额可能耗尽/风控'}，等待后换新会话继续（第 ${rot + 1} 次）${cookieLabel}...`);
          // close old browser
          try {
            if (s.browser) await Promise.race([s.browser.close(), new Promise(r => setTimeout(r, 5000))]).catch(() => { });
          } catch (e) { }
          await this.interruptibleSleep(memoryRestart ? 8000 : 30000); // memory restart needs less cool-down
          // reopen a fresh session with the (possibly new) cookie
          try {
            if (backup) { s.cookieFile = backup; rotatedCookie = true; }
            const reopened = await this.openSession(mode, s.cookieFile, s.index, sessionsIsolated);
            s.browser = reopened.browser;
            s.profileDir = reopened.profileDir;
            const lp2 = await openLandingPage(s.browser, s.cookieFile, () => this.stopped);
            s.page = lp2.page;
          ...
          res = await scrapeByInput(s.page, inputs, {
            ...
            onNewCreators: async (newOnes) => {
              ...
              for (const c of newOnes) {
                ...
                // timeout handled inside fetchOneDetail (cancelable)
                try { await fetchOneDetail(c); } catch (e) { }
                await this.interruptibleSleep(4000 + Math.random() * 5000); // tested-stable pacing (4-9s)
              }
            },
        ...
        // quota exhausted → rotate with a backup cookie; OR enough details were
        // fetched that a memory-restart is worthwhile → rotate with same cookie
        if (res && res.rateLimited) continue;
        if (detail && detailCount >= MEMORY_RESTART_DETAILS && !this.stopped) continue;
        break;
      }
      ...
      s.done = true;
      // any other session still running decides the phase; if none, clear it
      if (!(this.sessions || []).some(x => !x.done && x !== s)) this.detailActive = false;
    }
  }
  ...
  async openSession(mode, cookieFile, index, isolated) {
    if (mode === 'headless') {
      ...
      return { browser: l.browser, profileDir: l.profileDir };
    ...
    if (b) return { browser: b, profileDir: null };
  ...
  async consumeSharedQueue(s, detail, fetchOneDetail, resumeList) {
    ...
    while (!this.stopped && !this.detailStopped) {
      ...
      if (c) {
        ...
        const cid = c.creator_oecuid == null ? '' : String(c.creator_oecuid);
        try { await fetchOneDetail(c); } catch (e) { }
        await this.interruptibleSleep(3000 + Math.random() * 4000); // slightly faster than list pacing
      } else {
        idleRounds++;
  ...
  // ── speed monitor ──
  // Call periodically (from the status IPC). Computes a rolling "creators per
  // minute" from the cumulative total, plus the current phase so the UI can
  // explain why speed differs (list vs detail).
  updateRateInfo() {
    const total = this.collectedTotal();
    const now = Date.now();
    const st = this._rateState || (this._rateState = { lastTotal: 0, lastAt: 0, ratePerMin: 0 });
    if (st.lastAt && now > st.lastAt) {
      const dtMin = (now - st.lastAt) / 60000;
      const delta = total - st.lastTotal;
      if (dtMin > 0 && delta >= 0) st.ratePerMin = Math.round(delta / dtMin);
    }
    st.lastTotal = total;
    st.lastAt = now;
    this.currentInfo.ratePerMin = st.ratePerMin;
    // phase: detail mode is inherently ~5-8× slower per item than list paging
    this.currentInfo.phase = this.detailActive ? 'detail' : 'list';
    this.currentInfo.total = total;
    return this.currentInfo;
  }
  collectedTotal() {
    let total = 0;
    for (const s of this.sessions || []) total += (s.creators || []).length;
    return total;
  }

  sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

  // sleep that returns early (before ms elapses) once stop() is requested,
  // so a running scrape unwinds promptly when the user clicks 结束.
  async interruptibleSleep(ms) {
    const step = 250;
    let waited = 0;
    while (waited < ms) {
      if (this.stopped) return;
      await this.sleep(Math.min(step, ms - waited));
      waited += step;
    }
  }
  ...
  stop() {
    if (this.running) {
      ...
      // Close every open browser right away so in-flight page operations
      // (evaluate, goto, the 90s landing settle) fail fast instead of making
      // the user wait out their timeouts. Data already collected is kept.
      for (const s of this.sessions || []) {
        if (s.browser) {
          try { s.browser.close().catch(() => { }); } catch (e) { }
        }
