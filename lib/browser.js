...
async function openLandingPage(browser, cookieFile, isStopped) {
  ...
  await Promise.race([
    page.goto(landingUrl(), { waitUntil: 'domcontentloaded', timeout: 60000 }),
    new Promise((_, reject) => setTimeout(() => reject(new Error('goto-timeout')), 65000)),
  ]).catch(e => { /* landing may fail on risk-control; settle loop below handles it */ });
  ...
  for (let i = 0; i < 20; i++) {
    ...
    let len = 0;
    try {
      len = await Promise.race([
        page.evaluate(() => document.body ? document.body.innerText.length : 0),
        new Promise((_, reject) => setTimeout(() => reject(new Error('settle-timeout')), 8000)),
      ]);
    } catch (e) { break; }
    if (len > 300) break;
