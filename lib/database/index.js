...
class CreatorDatabase {
    ...
    try {
      for (const row of validRows) {
        ...
        await this.run(upsertSql, [
          ...
          canUpdate('avatar') ? scalar(row.avatar) : '',
  ...
  async listCreators(filters = {}) {
    ...
    const sortable = new Set(['handle', 'nickname', 'creator_id', 'region', 'follower_count', 'total_gmv', 'units_sold', 'contact_email', 'last_refreshed_at']);
    ...
    const rows = await this.all(
      `SELECT id, region, creator_id, handle, nickname, avatar_url, category,
              follower_count, total_gmv, units_sold, bio, contact_email, mcn,
              first_seen_at, last_refreshed_at, raw_json
