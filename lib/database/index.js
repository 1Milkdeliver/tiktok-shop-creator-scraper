...
class CreatorDatabase {
  ...
  _buildWhere(filters = {}) {
    ...
    for (const [key, values] of Object.entries(fieldFilters)) {
      ...
      for (const v of list) {
        ...
        // Column-backed fields: category and MCN live in dedicated columns.
        if (key === 'category') {
          clauses.push(`category LIKE ?`);
          params.push(`%${val}%`);
          continue;
        }
        if (key === '垂直类目' || key === 'vertical_category') {
          clauses.push(`vertical_category LIKE ?`);
          params.push(`%${val}%`);
          continue;
        }
        if (key === 'MCN机构' || key === 'mcn') {
          clauses.push(`mcn LIKE ?`);
          params.push(`%${val}%`);
          continue;
        }
  ...
  async getFilterOptions(key) {
    try {
      // Column-backed fields: some values live in dedicated columns, not raw_json.
      if (key === 'MCN机构' || key === 'mcn') {
        const rows = await this.all(`SELECT DISTINCT mcn FROM creators WHERE mcn IS NOT NULL AND mcn <> '' ORDER BY mcn LIMIT 200`);
        return rows.map(r => String(r.mcn)).filter(Boolean);
      }
      if (key === 'category') {
        const rows = await this.all(`SELECT category FROM creators WHERE category IS NOT NULL AND category <> ''`);
  ...
  // Build the category cascade from creator rows instead of presenting every
  // vertical category beneath every top-level category. A creator may expose
  // more than one value in either field, so each observed vertical value is
  // associated with every top-level value on that same creator record.
  async getCategoryTree() {
    try {
      const rows = await this.all(`
        SELECT category, vertical_category
        FROM creators
      ...
      for (const row of rows) {
        const parents = splitValues(row.category);
        const children = splitValues(row.vertical_category);
        for (const parent of parents) {
          if (!tree.has(parent)) tree.set(parent, new Set());
          const bucket = tree.get(parent);
          for (const child of children) bucket.add(child);
        }
      }

      return [...tree.entries()].slice(0, 200).map(([value, children]) => ({
        value,
        children: [...children].sort((a, b) => a.localeCompare(b)).slice(0, 200),
      }));
    } catch (e) { return []; }
  }
