  ...
  async upsertCreators(rows, options = {}) {
    ...
    const upsertSql = `
      INSERT INTO creators(
        ...
        last_publish_at, activity_status, activity_reason
      ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  _buildWhere(filters = {}) {
    ...
    for (const [key, values] of Object.entries(fieldFilters)) {
      ...
      for (const v of list) {
        ...
        // match JSON `"key":"value…` (prefix only — the value continues, e.g.
        // "Female: 79.47%"). A closing quote is NOT included because JSON
        // string values may contain more text after the token.
        clauses.push(`(raw_json LIKE ? OR raw_json LIKE ?)`);
        params.push(`%"${key}":"${val}%`, `%"${key}": "${val}%`);
      }
